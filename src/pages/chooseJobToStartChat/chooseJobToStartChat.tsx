...
const ChooseJobToStartChat = (props: any) => {
  ...
  const callJobList = async () => {
    ...
    const oppUsrId = new URLSearchParams(props.history?.location?.state).get(
      storageService.getItem("userType") === 1 ? "builderId" : "tradieId"
    );
              ...
              <div className="flex_row">
                <div className="flex_col_sm_5">
                  <div className="relate">
                    ...
                    ></button>
                    <span className="title">{"Choose the job"}</span>
                  </div>
                </div>

                {!props.isLoading && jobList?.length === 0 && (
                  <div className="no_record">
                    <figure className="no_img">
                      <img src={noData} alt="data not found" />
                    </figure>
                    {/* <span>No Jobs Found</span> */}
                    <span>{`You must have worked on at least a single job with the ${
                      storageService.getItem("userType") === 1
                        ? "builder"
                        : "tradesperson"
                    } to start conversation with them.`}</span>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="form_field">
                <button
                  ...
                  className={`fill_btn full_btn btn-effect ${
                    !Object.keys(editItem).length && !jobId && !jobName
                      ? "disable_btn"
                      : ""
                  }`}
                >
                      ...
                      return (
                        <li key={index}>
                          <div className="checkbox_wrap agree_check">
                            <input
                              checked={editItem[item?._id] ? true : false}
                              onChange={(e: any) => {
                                checkOnClick(e, item?._id, item?.jobName);
                              }}
