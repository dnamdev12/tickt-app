  ...
  redirectPath = (item: any) => {
    ...
    if (jobId && tradieId) {
      history.push({
        ...
        search: `?jobId=${jobId}&tradeId=${tradieId}&hideInvite=${
          hideInvite ? true : false
        }`,
        ...
        >
          <div className="f_spacebw tag_review">
            <span className="form_label">
              {item?.tradeData?.[0]?.tradeName ||
                item?.tradie_details?.trade?.[0]?.trade_name ||
                item?.trade?.[0]?.trade_name}
            </span>
            <span className="rating">
              {item?.ratings ||
                item?.rating?.toFixed(1) ||
                item?.tradie_details?.rating?.toFixed(1) ||
                "0"}{" "}
              |{" "}
              {item?.reviews ||
                item?.review ||
                item?.tradie_details?.review ||
                "0"}{" "}
              reviews{" "}
            </span>
          </div>
          ...
          <span
            ...
            className="more_detail new_top circle"
          ></span>
          <div className="user_wrap">
            <figure className="u_img">
              <img
                src={
                  item?.tradieImage ||
                  item?.tradie_details?.user_image ||
                  item?.user_image ||
                  dummy
                }
                alt="traide-img"
          ...
          {showStatus && item?.status && (
            <div className="form_field">
              <div className="job_status">{item?.status}</div>
            </div>
          )}
        </div>
      </div>
