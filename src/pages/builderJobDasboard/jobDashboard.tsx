...
class JobDashboard extends Component<Props, State> {
  ...
  checkAndUpdateCount = ({ needApprovalCount, newApplicantsCount }: any) => {
    let { approveCount, applicantCount } = this.state?.count;
    if (
      needApprovalCount !== approveCount ||
  ...
  componentDidUpdate() {
    ...
    console.log({
      ...
      approvalJobs,
      1: jobtype === "active",
      2: !activeJobs?.active?.length
        ? true
        : JSON.stringify(activeJobs?.active) !==
          JSON.stringify(this.state?.activeJobs),
    ...
    if (
      jobtype === "active" && !activeJobs?.active?.length
        ? true
        : JSON.stringify(activeJobs?.active) !==
            JSON.stringify(this.state?.activeJobs) &&
      ...
      if (activeJobs?.active) {
        ...
        if (
          Array.isArray(this.state?.activeJobs) &&
        ...
        if (
          ...
          !active?.length &&
          page_get === 0 &&
          this.state?.activeJobs?.length !== 0
        ) {
          ...
          let result = [];
          if (
            JSON.stringify(prevValues) === JSON.stringify(active) &&
            page_get === currentPage
          ) {
            ...
            if (firstItem?.jobId) {
              let ifMatch = prevValues.find(
                (item: any) => item.jobId === firstItem?.jobId
              );
              if (!ifMatch) {
            ...
            result =
              page_get > 0 && page_get === currentPage
                ? page_get == 1 && currentPage == 1
                  ? active
                  : concatedItems
                : active;
          }
          ...
          let randomState =
            ...
            Array.isArray(this.state.activeJobs) &&
            this.state.activeJobs[0] &&
            this.state.activeJobs[0].mathrandom
              ? this.state.activeJobs[0].mathrandom
              : "";

          let randomResult =
            result && Array.isArray(result) && result[0] && result[0].mathrandom
              ? result[0].mathrandom
              : "";
    ...
    if (
      jobtype === "open" && !openJobs?.open?.length
        ? true
        : JSON.stringify(openJobs?.open) !==
            JSON.stringify(this.state?.openJobs) &&
      ...
      if (openJobs?.open) {
        ...
        if (
          Array.isArray(this.state?.openJobs) &&
          this.state?.openJobs?.length
        ) {
        ...
        if (
          ...
          let result = [];
          if (
            JSON.stringify(prevValues) === JSON.stringify(open) &&
            page_get === currentPage
          ) {
            // same data items here!
            alert("Ok!");
          ...
          let randomState =
            this.state.openJobs &&
            Array.isArray(this.state.openJobs) &&
            this.state.openJobs[0] &&
            this.state.openJobs[0].mathrandom
              ? this.state.openJobs[0].mathrandom
              : "";
          ...
          if (
            this.state.count.approveCount !== needApprovalCount ||
    ...
    if (
      jobtype === "past" && !pastJobs?.past?.length
        ? true
        : JSON.stringify(pastJobs?.past) !==
            JSON.stringify(this.state?.pastJobs) &&
      ...
      if (pastJobs?.past) {
        ...
        if (
          Array.isArray(this.state?.pastJobs) &&
          this.state?.pastJobs?.length
        ) {
        ...
        if (
          ...
          let randomState =
            this.state.pastJobs &&
            Array.isArray(this.state.pastJobs) &&
            this.state.pastJobs[0] &&
            this.state.pastJobs[0].mathrandom
              ? this.state.pastJobs[0].mathrandom
              : "";
          let randomResult =
            result && Array.isArray(result) && result[0] && result[0].mathrandom
    ...
    if (
      ...
      JSON.stringify(applicantsListJobs) !==
        JSON.stringify(this.state.applicantsListJobs)
    ) {
    ...
    if (
      jobtype === "applicant" &&
      JSON.stringify(applicantJobs) !==
        JSON.stringify(this.state?.applicantJobs) &&
      ...
      if (
        Array.isArray(this.state?.applicantJobs) &&
        this.state?.applicantJobs?.length
      ) {
      ...
      if (
        hasLoad &&
        !applicantJobs?.length &&
        page_get === 0 &&
        this.state?.applicantJobs?.length !== 0
      ) {
        if (this.state.hasLoad !== false) {
          this.setState({ hasLoad: false });
        }
    ...
    if (
      jobtype === "approval" &&
      JSON.stringify(approvalJobs) !==
        JSON.stringify(this.state?.approvalJobs) &&
      ...
      if (
        Array.isArray(this.state?.approvalJobs) &&
        this.state?.approvalJobs?.length
      ) {
      ...
      if (
        ...
        !approvalJobs?.length &&
        page_get === 0 &&
        this.state?.approvalJobs?.length !== 0
      ) {
        if (this.state.hasLoad !== false) {
          this.setState({ hasLoad: false });
        }
