...
const MarkMilestones = (props: any) => {
  ...
  useEffect(() => {
    ...
    if (isForce) {
      props.history.replace(
        `${props.location.pathname}?active=${active}&jobId=${jobId}&markMilestone=${markMilestone}`
      );
  ...
  if (toggleItem?.lodge) {
    return <LodgeDispute item={selectedItem} backTab={backTab} />;
  }

  if (toggleItem?.cancel) {
    return <CancelJobs item={selectedItem} backTab={backTab} />;
  }

  let item_detail: any = itemDetails;
  let item_status = false;
  if (item_details?.milestones?.length) {
    let isExist = item_details?.milestones.find(
    ...
    <div className="flex_row">
      <div className="flex_col_sm_6">
        <div className="relate">
          ...
          <span className="xs_sub_title">
            {item_detail && Object.keys(item_detail).length
              ? item_detail.jobName
              : ""}
          </span>
              ...
              return (
                ...
                >
                  ...
                  <div className="info">
                    ...
                    {["check"].includes(classChecks(isActive, isPrevDone)) &&
                    expandItem[milestoneId] ? (
                      <button
                  ...
                  {["active"].includes(classChecks(isActive, isPrevDone)) &&
                  expandItem[milestoneId] ? (
                    <button
                      className="fill_btn full_btn btn-effect"
                      onClick={() => {
      ...
      <div className="flex_col_sm_6 col_ruler">
        ...
        <div
          onClick={() => {
            props?.history?.push(
              `tradie-info?tradeId=${item_details?.tradieId}&hideInvite=true&active=true&jobId=${item_details?.jobId}`
            );
          }}
        ...
        >
          ...
          <div className="user_wrap">
            ...
            <div className="details">
              <span className="name">
                {item_details?.tradie?.tradieName || ""}
              </span>
              {item_details?.tradie && (
                <span className="rating">
                  {item_details?.tradie?.ratings || 0} |{" "}
                  {item_details?.tradie?.reviews || 0} reviews
                </span>
              )}
            </div>
          </div>
