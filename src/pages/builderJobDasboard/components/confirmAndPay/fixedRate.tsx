...
const FixedRate = (props: any) => {
  ...
  const [toggleDetails, setToggleDetails] = useState<any>({
    toggle: false,
    data: null,
  });
    ...
    if (value) {
      return (
        <div className="f_spacebw">
          <span className="form_label">{bold ? <b>{title}</b> : title}</span>
          <span className="form_label">{bold ? <b>{value}</b> : value}</span>
        </div>
      );
    }
    return null;
    ...
    <div className="flex_row">
      <div className="flex_col_sm_8">
        ...
        <div className="form_field">
          <span className="sub_title">{"Confirm and pay"}</span>
        </div>
        {isLoading ? null : (
          <>
              ...
              <div>
                <div className="view_more agree_check mb50">
                  <input
                    onChange={() => {
                      checkPaymentType !== "1"
                        ? setCheckPaymentType("1")
                        : setCheckPaymentType("0");
                    }}
