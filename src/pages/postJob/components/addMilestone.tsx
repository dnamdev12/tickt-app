...
export default class AddMilestone extends Component<Props, State> {
  ...
  setItems = (is_remove?: boolean) => {
    ...
    let { milestone_name, isPhotoevidence, recommended_hours } =
      this.state;
    let milestone_index = milestones.length ? milestones.length - 1 : 0;
  ...
  render() {
    ...
    if (milestones.length) {
      ...
      if (date_from_moment?.length) {
        from_date_format = date_from_moment;
      }

      if (date_to_moment?.length) {
        to_date_format = date_to_moment; 
      }
    }
            ...
            >
              ...
              <DialogActions>
                ...
                <Button
                  onClick={() => {
                    if (
                      milestone_name?.length ||
                      isPhotoevidence == true ||
                      recommended_hours?.length ||
                      from_date_format?.length
                    ) {
