...
const JobDetails = ({
  ...
  const renderByFileFormat = (data: any) => {
    ...
    if (data_clone?.urls?.length) {
      let format_items = data_clone?.urls
        ?.map((item: any) => {
          let split_item_format = item?.link?.split(".");
          let get_split_fromat =
            split_item_format[split_item_format.length - 1];

          if (
            imageFormats.includes(get_split_fromat) ||
            videoFormats.includes(get_split_fromat)
          ) {
            return {
              url: item?.link,
