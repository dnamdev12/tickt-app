...
export default class EditMilestone extends Component<Props, State> {
  ...
  checkIsDateValid = (milestones: any, time: any) => {
    ...
    if (milestones.length > 1) {
      filterMilestone = milestones.filter(
        (index: any) => index !== editMileStone
      );
      filterMilestone.forEach((mile: any) => {
        ...
        if (validStart && validEnd) {
          if (validStartInput && validEndInput) {
            if (
              ...
              (moment(time.to_date).isSameOrBefore(mile.to_date) ||
        ...
        if (validStart && validStartInput && !validEnd) {
          if (
            moment(time.from_date).isSame(mile.from_date) ||
            moment(time.to_date).isSame(mile.from_date)
          ) {
            checkIsValid = false;
            isSkip = false;
          }
  ...
  componentDidMount() {
    ...
    if (item && Object.keys(item).length) {
      ...
      if (editMilestoneTiming && Object.keys(editMilestoneTiming).length) {
        const { checkIsValid, skip } = this.checkIsDateValid(
          milestones,
          editMilestoneTiming
        );
      ...
      this.setState(
        {
          from_date: from_date,
          isPhotoevidence:
            isPhotoevidence === undefined ? false : isPhotoevidence,
  ...
  checkErrors = () => {
    ...
    if (milestone_name?.length) {
      ...
      if (
        !error_1?.length &&
        (recommended_hours?.length === 0 || !pattern_error?.length)
      ) {
        return false;
      }
    }
  ...
  render() {
    const { handleStepForward,editMileStone } =
      this.props;
    let {
      milestone_name,
      isPhotoevidence,
              ...
              <div className="flex_col_sm_5">
                ...
                <div className="form_field">
                        ...
                        this.setState(
                          ...
                          () => {
                            ...
                            if (
                              ...
                              if (!conditionItem) {
                                error_item["pattern_error"] =
                                  "Time should be in multiples of 15 like 10:15, 10:30";
                              }
                            } else {
                              error_item["pattern_error"] =
                                "Please enter a valid pattern like : 10:15";
                            }
                  ...
                  <span className="error_msg">
                    {recommended_hours?.length === 0
                      ? ""
                      : errors.recommended_hours}
                  </span>
                  <span className="error_msg">
                    {recommended_hours?.length === 0
                      ? ""
