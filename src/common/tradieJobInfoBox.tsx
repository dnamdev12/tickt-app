...
const TradieJobInfoBox = (props: any) => {
  const jobClickHandler = (items: any) => {
    if (storageService.getItem("userType") === 1) {
      let string_redirect: any = `/job-details-page?jobId=${items.jobId}`;
      if (items?.tradeId) {
        string_redirect += `&tradeId=${items.tradeId}`;
      }
      if (items?.specializationId) {
        string_redirect += `&specializationId=${items.specializationId}`;
      }
      if (items?.jobStatus) {
        string_redirect += `&jobStatus=${items.jobStatus}`;
      }
      console.log({ items, string_redirect });
      props.history.push(string_redirect);
    } else {
      let status = "";
      if (items?.jobStatus) {
        status = (items?.jobStatus).toLowerCase();
      }

      let string_items = `?jobId=${items?.jobId}&status=${status}&edit=true`;
      if (["expired", "completed"].includes(status)) {
        string_items += `&job=past`;
      }
      console.log({
        string_items,
        status,
      ...
      let urlEncode: any = string_items;
        ...
        <div className="user_wrap">
          <figure className={`u_img ${props.userType === 2 ? "icon" : ""}`}>
            <img
              src={
                props.userType === 2
                  ? item.tradeSelectedUrl
                  : item.builderImage || dummy
              }
              alt=""
              onError={(e: any) => {
                if (e?.target?.onerror) {
          ...
          <div className="details">
            <span className="name">
              {props.userType === 2 ? item.tradeName : item.jobName}
            </span>
            <span className="prof">
              {props.userType === 2 ? item.jobName : item.builderName}
            </span>
          </div>
        </div>
